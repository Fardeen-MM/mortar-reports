...
Instantly: Lead marked "Interested"
        ↓
Instantly webhook → https://mortar-instantly-webhook.fardeen-729.workers.dev
        ↓
...
- ✅ Worker accepts webhooks
- ✅ Worker forwards to GitHub (returns 204)
- ⏳ Verifying GitHub Actions triggers correctly
...
4. **Test the entire chain** - webhook → worker → GitHub → workflow
...
**Key insight:** Every time we update the system, we assume webhooks work because they "worked before" - but they were never properly tested end-to-end with the actual Cloudflare Worker → GitHub flow.
