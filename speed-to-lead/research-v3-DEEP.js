...
// Google Places API for competitor finding
...
async function deepResearch(firmWebsite, contactName = '') {
  ...
  try {
    ...
    console.log(`   ⚖️  Practice areas found: ${research.practiceAreas.length}`);
    ...
    // ========================================================================
    // STEP 3: ANALYZE ABOUT PAGE
    // ========================================================================
    if (pagesToVisit.about) {
      console.log(`\n📖 Step 3: Analyzing About page...`);
      try {
    ...
    // ========================================================================
    // STEP 4: ANALYZE TEAM/ATTORNEYS PAGE (DEEP EXTRACTION)
    // ========================================================================
    if (pagesToVisit.team) {
      console.log(`\n👥 Step 4: Analyzing Team page...`);
      try {
    ...
    // ========================================================================
    // STEP 5: EXTRACT ALL LOCATIONS (HOMEPAGE + CONTACT PAGE)
    // ========================================================================
    console.log(`\n📍 Step 5: Extracting all locations...`);
    ...
    try {
        ...
        if (research.allLocations.length > 0) {
          research.location = research.allLocations[0];
        }
        
        console.log(`   📍 Locations found: ${research.allLocations.length}`);
    ...
    // ========================================================================
    // STEP 6: FIND COMPETITORS VIA GOOGLE PLACES API
    // ========================================================================
    console.log(`\n🔍 Step 6: Finding real competitors via Google Places API...`);
    
    try {
      // Check if we have at least one valid location
      const hasValidLocation = research.location.city && research.location.city.length > 0;
      ...
      if (!hasValidLocation) {
        ...
        if (research.location.country === 'US') {
          // US: "City, State" (e.g., "Denver, Colorado")
          const stateFullName = {
          ...
          // Canada: "City, Province, Canada" (to avoid confusion with California)
          ...
          // UK: "City, United Kingdom"
          searchLocation = `${research.location.city}, United Kingdom`;
        } else {
          // Other countries: "City, Country Name"
          const countryNames = {
            'Spain': 'Spain',
            'France': 'France',
            'Germany': 'Germany',
      ...
      if (placesResult.status === 'OK' && placesResult.results) {
        ...
        for (const place of placesResult.results) {
          ...
          if (research.location.country === 'US') {
            // Check if address contains "USA" or state abbreviation
      ...
      } // end of else block for valid location
    ...
    // ========================================================================
    // STEP 7: FIND LINKEDIN PROFILES
    // ========================================================================
    if (contactName && contactName.length > 0) {
      console.log(`\n🔗 Step 7: Finding LinkedIn profile for ${contactName}...`);
    ...
    // ========================================================================
    // STEP 8: CHECK GOOGLE/META ADS (ACCURATE VERIFICATION)
    // ========================================================================
    console.log(`\n📱 Step 8: Checking advertising presence...`);
    ...
    try {
      ...
      // ONLY use DOM fallback if text extraction completely failed (no match at all)
      // Don't use fallback if we successfully extracted "0 ads"
      if (!adCountMatch) {
        // Try more specific selectors for ad elements
        const selectors = [
          '[data-creative-id]',
          '.creative-preview',
          '[class*="ad-card"]',
        ...
        for (const selector of selectors) {
          const count = await googleAdsPage.locator(selector).count();
          if (count > adElementCount) {
            adElementCount = count;
          }
        }
      }
      ...
      if (!isRunningAds) {
        ...
        console.log(`   ✅ Google Ads: Running${adElementCount > 0 ? ` (${adElementCount} ad(s) detected)` : ''}`);
    ...
    if (facebookPageUrl) {
      ...
      try {
        ...
        // First check if the URL itself contains the ID (e.g., profile.php?id=123456)
        ...
        if (!facebookPageId) {
          // Visit the page and extract ID
          await fbPage.goto(facebookPageUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
          await fbPage.waitForTimeout(5000); // Wait for dynamic content
          
          const pageSource = await fbPage.content();
          
          // Method 1: Look for "page_id":"123456789"
          let pageIdMatch = pageSource.match(/"page_id":"(\d+)"/);
          if (pageIdMatch) {
            facebookPageId = pageIdMatch[1];
            console.log(`   ✅ Facebook Page ID (method 1): ${facebookPageId}`);
          }
          
          // Method 2: Look for "pageID":"123456789"
          if (!facebookPageId) {
            pageIdMatch = pageSource.match(/"pageID":"(\d+)"/);
            if (pageIdMatch) {
              facebookPageId = pageIdMatch[1];
              console.log(`   ✅ Facebook Page ID (method 2): ${facebookPageId}`);
            }
          }
          
          // Method 3: Look for "entity_id":"123456789"
          if (!facebookPageId) {
            pageIdMatch = pageSource.match(/"entity_id":"(\d+)"/);
            if (pageIdMatch) {
              facebookPageId = pageIdMatch[1];
              console.log(`   ✅ Facebook Page ID (method 3): ${facebookPageId}`);
            }
          }
          
          // Method 4: Look for "owning_profile":{"__typename":"Page","id":"123456789"}
          if (!facebookPageId) {
            pageIdMatch = pageSource.match(/"owning_profile":\{"__typename":"Page","id":"(\d+)"/);
            if (pageIdMatch) {
              facebookPageId = pageIdMatch[1];
              console.log(`   ✅ Facebook Page ID (method 4): ${facebookPageId}`);
            }
          }
          
          // Method 5: Look for contentID or profileID
          if (!facebookPageId) {
            pageIdMatch = pageSource.match(/"contentID":"(\d+)"/);
            if (pageIdMatch) {
              facebookPageId = pageIdMatch[1];
              console.log(`   ✅ Facebook Page ID (method 5): ${facebookPageId}`);
            }
          }
          ...
          if (!facebookPageId) {
            try {
              const metaContent = await fbPage.locator('meta[property="al:android:url"]').getAttribute('content');
              if (metaContent) {
                const match = metaContent.match(/fb:\/\/page[\/\?](\d+)/);
                if (match) {
                  facebookPageId = match[1];
                  console.log(`   ✅ Facebook Page ID (method 6): ${facebookPageId}`);
                }
              }
            } catch (e) {
              // Meta tag not found
          ...
          // Method 7: Try to find in any data-gt attribute or similar
          if (!facebookPageId) {
            // Sometimes Page ID is in data attributes
            const allPageIds = pageSource.match(/\b(\d{15,16})\b/g); // Facebook IDs are typically 15-16 digits
            if (allPageIds && allPageIds.length > 0) {
              // Take the first one that appears multiple times (likely the page ID)
              const idCounts = {};
              for (const id of allPageIds) {
                idCounts[id] = (idCounts[id] || 0) + 1;
              }
              const sortedIds = Object.entries(idCounts).sort((a, b) => b[1] - a[1]);
              if (sortedIds.length > 0 && sortedIds[0][1] >= 3) {
                facebookPageId = sortedIds[0][0];
                console.log(`   ✅ Facebook Page ID (method 7): ${facebookPageId}`);
              }
            }
          }
        }
        ...
        if (!facebookPageId) {
          console.log(`   ⚠️  Could not extract Facebook Page ID from: ${facebookPageUrl}`);
        }
    ...
    if (facebookPageId) {
      ...
      try {
        ...
        // Extract active ad count from text like "5 results" or "~3 results"
        let activeAdCount = 0;
        ...
        // SECOND: Check for ALL ads (including inactive)
      ...
      // No Page ID found, assume not running ads
      research.gaps.metaAds.hasGap = true;
      research.gaps.metaAds.impact = 12000;
...
if (require.main === module) {
  ...
  if (!url) {
    console.log('Usage: node research-v3-DEEP.js <website-url> [contact-name]');
    console.log('Example: node research-v3-DEEP.js https://www.aklaw.net "Paul A. Graziano"');
    process.exit(1);
  }
  
  deepResearch(url, contactName).then(() => process.exit(0)).catch(err => {
    console.error(err);
    process.exit(1);
  });
}
