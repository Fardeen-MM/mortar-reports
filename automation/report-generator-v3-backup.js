...

{
  ...
  "totalStripContext": "One paragraph after the total strip. Reinforce the guarantee. We guarantee these results. The range depends on case values and close rate but the opportunity is real and we stand behind it.",
  ...
  "ctaHeadline": "Headline for the booking CTA. Reference the guarantee. Confident.",
  "ctaSubtext": "One sentence. 15 minutes, we guarantee results, low-pressure."
}`;
...
function generateFallbackProse(context) {
  ...
  return {
    ...
    totalStripContext: `We guarantee these results. The exact number depends on your case values and close rate, but the opportunity is real and we stand behind it. Real people in ${locationStr} are searching for the exact service you provide, and we'll make sure they find you.`,
    ...
    buildItem4Timeline: 'Set up alongside launch',
    ctaHeadline: `We guarantee results. Let's talk.`,
    ctaSubtext: `15 minutes. We'll show you exactly how we'll deliver these cases for your ${lawFirmWord}. Guaranteed.`
  };
}
...
<body>
  <div class="container">
    ...
    <div class="total-strip">
      <div class="total-strip-text">Guaranteed monthly opportunity</div>
...
${generateTwoClientVisual(clientLabel, runningMetaAds, metaAdCount, country)}
    ...
    <div class="case-study fade-in">
      <div class="case-study-label">Real results</div>
...
function calculateGap1(marketMultiplier, caseValues, countryBaseline, currency) {
  ...
  const minLow = Math.round(18000 * countryBaseline / 500) * 500 || 500;
  const minHigh = Math.round(32000 * countryBaseline / 500) * 500 || 1000;
  return {
...
function calculateGap3(firmSizeMultiplier, caseValues, countryBaseline, currency) {
  ...
  const low = Math.round(casesPerMonth * caseValues.low / 500) * 500;
  const high = Math.round(casesPerMonth * caseValues.high / 500) * 500;
  const minLow = Math.round(14000 * countryBaseline / 500) * 500 || 500;
  const minHigh = Math.round(26000 * countryBaseline / 500) * 500 || 1000;
  return {
...
function validateReportHTML(html, firmName) {
  ...
  const sections = [
    ['hero', /class="hero"/i],
    ['gap-search', /class="gap-card\s+gap-search/i],
    ['gap-social', /class="gap-card\s+gap-social/i],
    ['gap-intake', /class="gap-card\s+gap-intake/i],
    ['total-strip', /class="total-strip"/i],
    ['case-study', /class="case-study/i],
    ['competitor-section', /class="competitor-section/i],
    ['build-list', /class="build-list/i],
  ...
  // 3 gap stat numbers exist
  const gapStats = html.match(/class="gap-stat-number"/g) || [];
  if (gapStats.length < 3) warnings.push(`Only ${gapStats.length} gap stat numbers (expected 3)`);
